...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 重启 UI 流程(转圈+读秒+心跳; 5s 检测延迟, 总 25s) ──
    ...
    function startRestartFlow(ov) {
      ...
      ov.innerHTML = '<div style="text-align:center;color:#fff">' +
        ...
        '<div data-count style="font-size:13px;color:#9a9aaa">最多等待 25s</div></div>'
      var n = 25
      var done = false
      var countEl = ov.querySelector('[data-count]')
      var iv = setInterval(function () {
        n--
        if (countEl) countEl.textContent = '最多等待 ' + n + 's'
        if (n > 20) return
        fetch('/', { method: 'GET' }).then(function (r) {
          ...
          if (r.status >= 200 && r.status < 500) {
            ...
            clearInterval(iv)
            ov.innerHTML = '<div style="text-align:center;color:#fff"><div style="font-size:26px;margin-bottom:8px">✓</div><div style="font-size:15px">已就绪</div></div>'
    ...
    // ── 进阶重启: 感知运行中会话, 分级确认 ──
    function requestRestart(fromAgent) {
      var others = 0
      try {
        ...
        if (sess && sess.list) {
          ...
          for (var i = 0; i < ids.length; i++) {
            var s0 = byId[ids[i]]
            if (s0 && !s0.blank && (s0.running || s0.pendingInteraction) && (s0.id || '') !== (cur || '')) others++
          }
        }
      } catch (e) {}
      if (others === 0) {
        try { ensureRestartUI() } catch (e) {}
        fetch('/api/dshm-restart-go', { method: 'POST' }).catch(function () {})
      } else {
        try { showConfirmUI(fromAgent === true) } catch (e) {}
      }
    }

    // 确认窗: 标题按来源; 是 → 二次确认(有运行黄色警告 / 无运行无标题简单)
    function showConfirmUI(fromAgent) {
      var ov = document.createElement('div')
      ...
      try {
        ...
        if (sess && sess.list) {
          ...
          runOthers = run.filter(function (x) { return (x.id || '') !== (curId || '') })
        }
      } catch (e) {}
      var wEl = ov.querySelector('[data-warn]')
      var rEl = ov.querySelector('[data-running]')
      if (run.length > 0) {
        wEl.innerHTML = '<div style="color:#e6b94c;font-weight:600;margin-bottom:6px">当前有 ' + run.length + ' 个对话正在运行</div>' +
        ...
        var rh = '<div style="color:#8a8a99;font-weight:600;margin-bottom:8px">正在运行</div>'
      ...
      var doSecond = function () {
        ...
        if (runOthers.length > 0) {
          var oh = '<div style="width:min(300px,88vw);background:#2a2018;border:1px solid #e6b94c55;border-radius:14px;padding:20px;box-shadow:0 12px 40px rgba(0,0,0,.5);text-align:center">' +
            '<div style="font-size:16px;font-weight:600;color:#e6b94c;margin-bottom:8px">⚠ 将中断其他运行中对话</div>' +
            '<div style="font-size:13px;color:#c8a86a;margin-bottom:10px">另有 ' + runOthers.length + ' 个对话正在运行，重启会中断它们（含 AI 工具调用）</div>'
          var shown = Math.min(runOthers.length, 5)
          for (var k2 = 0; k2 < shown; k2++) {
            var t2 = (runOthers[k2].title || '').trim() || String(runOthers[k2].id || '').slice(0, 8)
            oh += '<div style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:12px;color:#e6d5b0;min-width:0;text-align:left">' +
              '<span style="width:8px;height:8px;border-radius:50%;border:2px solid #e6b94c;border-top-color:transparent;animation:dshmSpin .8s linear infinite;flex:none"></span>' +
          ...
          oh += '<div style="display:flex;gap:10px;margin-top:14px">' +
            ...
            '</div></div>'
          ov.innerHTML = oh
          ov.querySelector('[data-back]').addEventListener('click', function () { if (ov.parentNode) ov.remove() })
      ...
      ov.querySelector('[data-yes]').addEventListener('click', doSecond)
    ...
    function ReloadButton() {
      ...
      return React.createElement('button', {
        ...
        title: '点击重载前端 · 长按/右键重载重启',
        onClick: function () { if (!fired.current) location.reload() },
    ...
    function apply(ctx) {
      ...
      try {
        var es = new EventSource('/api/dshm-reload-sse')
        es.addEventListener('message', function (e) {
          if (e.data === 'reload') location.reload()
          else if (e.data === 'restart') requestRestart(true)
        })
      } catch (e) {}
